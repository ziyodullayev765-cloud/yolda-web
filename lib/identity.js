...
const saveTelegramProfile = async (tgUser, identity) => {
  ...
  // Reverse index, so lib/notify.js can go the other way — from an
  // identity to a chat the bot may message. Written on every Telegram
  // login and on link redemption, which is also what repairs it after
  // notify.js drops a chat that blocked the bot.
  await kvSet(`tgChat:${identity}`, String(tgUser.id));
};
